...
const GRAPHIC_TYPE = 'Hailing Port';


const HailingPort = ({ w }) => {
    const b = useGlobalState();

    const hailing_port = b[GRAPHIC_TYPE];
    ...
    return (
        <div className="bg-white p-4 container">
            ...
            <label htmlFor={`${GRAPHIC_TYPE}_text`}>
                <span className="text-primary font-bold">City & State</span>
                <span className="block text-xs text-gray-500 italic">Provide the city and state where your boat is registered</span>
                <input
                    type="text"
                    id={`${GRAPHIC_TYPE}_text`}
                    // onChange={e => { setValue(e.target.value); }}
                    // onChange={e => b.updateState('Boat Name', { text: e.target.value })}
                    // className="p-2 mt-3"
                    value={hailing_port.text}
                ...
                <div className="flex gap-5 mt-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 pt-1">
                            <p className="text-sm">{hailing_port.selectedFont}</p>
                            <button
                                type="button"
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 pt-1">
                            <p className="text-sm">{hailing_port.color.name}</p>
                            <button
                                type="button"
                                className="text-xs text-blue-700"
                            >
            ...
            <div className="pt-8">
                ...
                <div className="flex gap-3 mt-3 g-green-100">
                    <label htmlFor="boatname_height">
                        ...
                        <CurrencyInput
                            value={hailing_port.imageSize.height}
                            suffix='"'
                            decimalScale={2}
                    ...
                    <label htmlFor="boatname_width">
                        ...
                        <CurrencyInput
                            value={hailing_port.imageSize.width}
                            suffix='"'
                            decimalScale={2}
                            allowNegativeValue={false}
                            onValueChange={value => {
                    ...
                    <label htmlFor="boatname_proportional">
                        ...
                        <div className="mt-2.5">
                            <Switch
                                checked={hailing_port.imageSize.maintainProportions}
                                onChange={() => b.updateState(GRAPHIC_TYPE, { imageSize: { ...hailing_port.imageSize, maintainProportions: !hailing_port.imageSize.maintainProportions } })}
                                className={`${hailing_port.imageSize.maintainProportions ? 'bg-secondary' : 'bg-gray-200'} relative inline-flex h-9 w-[4.5rem] items-center rounded-full`}
                            >
                                <span className="sr-only">Enable notifications</span>
                                <span
                                    // className={`${hailing_port.imageSize.maintainProportions ? 'translate-x-[2.5rem]' : 'translate-x-[0.25rem]'} absolute inline-block h-7 w-7 transform rounded-full bg-white transition`}
                                    className={`${hailing_port.imageSize.maintainProportions ? 'translate-x-10' : 'translate-x-1'} absolute inline-block h-7 w-7 transform rounded-full bg-white transition`}
                                />
                ...
                <div className={`pt-2 transition-all ${hailing_port.imageSize.maintainProportions ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
